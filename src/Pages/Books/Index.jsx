import { Button, Col, Divider, Empty, Input, List, Row, Select, Space, Spin, Statistic } from "antd";
...
const Index = () => {
	...
	const { data, isLoading, isValidating, mutate } = useSWR(`/api/books?${queryString}`, null, {revalidateOnFocus: false})
				...
				<Col span={6}>
					<Space direction="vertical">
						...
						<Select
							...
							onSelect={value => { searchParams.set("authorId", value); setSearchParams(searchParams); mutate() }}
							onClear={() => { searchParams.delete("authorId"); setSearchParams(searchParams); }}
							options={authors.map((x) => ({ label: `${x.firstName} ${x.patronymic ?? ""} ${x.lastName}`, value: x.id }))} />
						<Select
							allowClear
							showSearch
							optionFilterProp="label"
							...
							onSelect={value => { searchParams.set("genreId", value); setSearchParams(searchParams); mutate() }}
							onClear={() => { searchParams.delete("genreId"); setSearchParams(searchParams); }}
